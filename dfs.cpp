...
bool DFSCode::toGraph (Graph &g)
{
	...
	for (DFSCode::iterator it = begin(); it != end(); ++it) {
		...
		g[it->from].edge.back().id = id;
		g[it->to].edge.back().id = id;
		++id;
	}
	g._setEdgesize(id);
	return (true);
...
DFSCode DFSCode::operator=(const DFSCode & rhs) {
	(*this).clear();
	(*this).reserve(rhs.size());
	for (std::vector <DFS> ::const_iterator IT = rhs.begin(); IT != rhs.end(); IT++) {
		(*this).push(IT->from, IT->to, IT->fromlabel, IT->elabel, IT->tolabel);
	}
	(*this).rmpath = rhs.rmpath;
	return *this;
};
