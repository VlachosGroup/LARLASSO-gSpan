...
void History::build (Graph &graph, PDFS *e)
{
	...
	// This makes a logical index of whether each edges and vertex has been accounted or not. 
	clear ();
	edge.clear ();
...
{
	result.clear (); // cleared everytime
	for (Vertex::edge_iterator it = v.edge.begin(); it != v.edge.end(); ++it) { // fot each edge
		assert (it->to >= 0 && it->to < g.size ()); // error check that index of connected vertex is larger than 0, and less than the max number of graph.
		if (v.label <= g[it->to].label) // if vertex label is lower than connected vertex label. Basically prevent from going backward
			result.push_back (&(*it)); // insert at the end. 
	}

	return (! result.empty()); // True: not empty, False: Empty
}
